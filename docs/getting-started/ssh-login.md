...
If your command would run a little slow on an inexpensive laptop from 2002, you shouldn't run it on the login node! Instead, request an [**interactive job**](https://hyak.uw.edu/docs/systems/klone/scheduling-jobs#interactive-jobs-single-node) on a compute node.
