...
The official guide to compile on centos is given [**here**](https://trac.ffmpeg.org/wiki/CompilationGuide/Centos/). It can be noticed right away how complex such compilation would be to simply be able to run the ffmpeg executable as many prerequisites are needed (nasm, yasm, etc...).
