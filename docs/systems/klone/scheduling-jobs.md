...
If you are new to Hyak and using the job scheduler, Slurm, you may find our Slurm tutorial helpful to walk you through basic and advanced usage. [**Click here to jump to the tutorial.**](https://hyak.uw.edu/docs/tutorials/slurm/syllabus)
This tutorial can also be followed in video form [**HERE**](https://www.youtube.com/watch?v=iYM7xpRhp8I).
:::

## Compute Resources

The Slurm scheduler has two high-level concepts you need to know, [**accounts**](/docs/systems/klone/get-started#accounts-and-partitions) and [**partitions**](/docs/systems/klone/get-started#partition-overview).

With the `hyakalloc` command [**source code here**](/docs/systems/klone/resource-monitoring#hyakalloc) you can further see not only which accounts you are able to submit jobs to but also their current utilization. Resource limits are directly proportional to what was contributed by that group.
...
* [**interactive**](#interactive-jobs-single-node) where you and test out your workflows live,
...
| - | - | - |
| Account | `-A` or `--account` | What lab are you part of? If you run the `groups` command you can see what groups (usually labs) you're a member of, these are associated with resource limits on the cluster. |
| Partition | `-p` or `--partition` | What resource partition are you interested in using? This could be anything you see when you run `sinfo -s` as each partition corresponds to a class of nodes (e.g., high memory, GPU). |
...
| Cores | `-c` or `--cpus-per-task` | How many compute cores do you need? Not all codes can make use of multiple cores and if they do, the performance of the code is not always linear with the resources requested. |
...
## Using Idle Resources (Checkpoint)
...
:::info
[**DMTCP**](/docs/guides/software/dmtcp) is a recommended tool for checkpointing many types of jobs on Hyak without modifying application code. See our [**documentation**](/docs/guides/software/dmtcp) for more information.
:::
...
- [**`sstat`**](https://slurm.schedmd.com/sstat.html) displays status information of a running job pertaining to CPU, Task, Node, Resident Set Size (RSS), and Virtual Memory (VM) statistics.
- [**`sacct`**](https://slurm.schedmd.com/sacct.html) displays information about completed jobs.
- [**`sreport`**](https://slurm.schedmd.com/sreport.html) generates reports about job usage and cluster utilization from Slurm accounting (`sacct`) data.
...
Gardner, Dylan, Robben Migacz, and Brian Haymore. "Arbiter: Dynamically Limiting Resource Consumption on Login Nodes." Proceedings of the Practice and Experience in Advanced Research Computing on Rise of the Machines (learning). 2019. 1-7. [**DOI: 10.1145/3332186.3333043**](https://doi.org/10.1145/3332186.3333043) [**Code: Gitlab**](https://gitlab.chpc.utah.edu/arbiter2/arbiter2)
