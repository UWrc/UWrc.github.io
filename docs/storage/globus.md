...
Globus public sharing is available with [**Kopah S3 storage**](https://hyak.uw.edu/docs/storage/gui#globus), which we recommend as a complement to your research storage portfolio if you anticipate requiring regular sharing and collaboration.
:::
...
* In the **File Manager - Collection Search** tab, look for "UW Hyak Klone" for `/gscratch/`.
...
* Your home directory will be the default filesystem path when you start Globus. Your path on the filesystem can be entered manually to change directories, allowing you to see directories that you have permission to access. The three-line menu can be revealed to show helpful features.

<img src="/img/docs/globus/1_file_manager.png" alt = "Image shows the user's home directory by default and indicates the three-line menu with helpful features. The path can be edited." />
...
Globus has a two-pane view option that allows you to see two collections in the same window and perform transfers between them. With UW Hyak Klone (`/gscratch/`) in one pane, click search in the other to select a different collection.
