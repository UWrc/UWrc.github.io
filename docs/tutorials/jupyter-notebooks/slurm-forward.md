...
The last line of the script executes `launch-container.sh` and `start-jupyter-server.sh` which will: start the container (with the read-write version of the container launcher), and have that container run our Jupyter. 
...
[**Flexible Connections EXTRA CREDIT**](https://hyak.uw.edu/docs/tutorials/jupyter-notebooks/ssh#windows-users-a-set-of-customized-local-configurations)

[**Jump to Next Step**](https://hyak.uw.edu/docs/tutorials/jupyter-notebooks/slurm-forward#a-script-for-ssh-port-forwarding)
...
I swear, this is the last one. [**You can download it to your local computer by clicking here**](https://hyak.uw.edu/files/hyak101/python/start-jupyter-forwarding.sh). Don't forget to make the script executable after you download it. 
