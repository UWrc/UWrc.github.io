...
As with previous exercises, you can use `apptainer shell` to open a shell into `ubuntu_latest.sif` and execute commands interactively.
...
Next, we will demonstrate how to convert the container into a sandbox so that additional software can be installed into it interactively.
