...
Globus public sharing is available with [**Kopah S3 storage**](/docs/systems/kopah/tools/gui#globus), which we recommend as a complement for your research storage portfolio if you anticipate requiring regular sharing and collaboration. 
:::
...
* In the **File Manager - Collection Search** tab, look for "UW Hyak Klone" for `/mmfs1/`, which contains `/gscratch/`.
...
* Your home directory will be the default filesystem path when you start Globus. Your path on the filesystem and be entered manually to change directories, allowing you to see directories that you have permission to access. The three-line menu can be revealed to show helpful features. 
