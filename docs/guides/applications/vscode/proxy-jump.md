...
**WARNING this script doesn't usually work on Windows since bash and sed are not available. It might work if you're on Windows with WSL or Gitbash. MOREOVER, it might not work if you have a different version of sed**
...
Via this method, Windows users may have to provide two-factor authentication when changing directories. A better alternative for Windows users might be to connect [**VS Code via Code-server**](code-server).
