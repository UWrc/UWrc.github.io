...
## Mathematica on Hyak
To start up Mathematica on Hyak, launch a job on a compute node with the [**salloc**](https://hyak.uw.edu/docs/systems/klone/scheduling-jobs#interactive-jobs-single-node) command. All modules, including Mathematica, are unavailable on login nodes. 
```js
# using the salloc command to switch from a login node to a compute node
salloc
```
Next, use [**LMOD**](https://hyak.uw.edu/docs/guides/software/modules) to load the Mathematica module:
```js
...
```
:::note 
To start the kernel, the module must retrieve the license for Mathematica from the Physics Department. This can take anywhere from 30 seconds to 1 minute. For more information about the university's Mathematica license, click [**HERE**](https://phys.washington.edu/mathematica). 
:::
