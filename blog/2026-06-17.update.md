...
Historically, projects were represented by a single Slurm account. Going forward, each project allocation is represented internally by multiple resource-specific accounts.
...
Following the Slurm modernization, the default output of commands such as `squeue` and `sacct` would normally have changed.

***To preserve continuity, we implemented compatibility wrapper scripts that maintain the familiar output formats most users expect.***

As a result, the majority of users should see little or no difference in their day-to-day workflows.

Advanced users should note that invoking these commands through their full system paths bypasses the compatibility wrappers and displays the native output provided by the updated Slurm configuration.
...
UW Groups display names and descriptions have been updated to guide group member manager on the type of access that is granted with group membership. 

### Storage and Fileset Permissions

In the majority of cases, Klone and Tillicum allocations are accompanied by storage allocations on that cluster, and enrollment in the cluster login will automatically grant permissions to filesets and storage resources. 
...
Group member managers, who manage project memberships, should be aware that login access and storage-only permissions are now managed separately. This is only relevant for the special use cases described above and many member managers will see not change in their member manager practices. 
...
Our next maintenance window for Klone and Tillicum is acheduled for Tuesday July 7, 2026. 
